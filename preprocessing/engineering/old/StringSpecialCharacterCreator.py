...
class StringSpecialCharacterCreator(BaseEstimator, TransformerMixin):
    """
    Klasse, welche die Anzahl an Sonderzeichen zählt.
    """
    def __init__(self):
        self.column_names = []

    def len_special_characters(self,X):
    ...
    def fit(self, X, y=None):
        ...
        for i in range(len(X.columns)):
            self.column_names.append(X.columns[i]+"_LEN_SPECIAL_"+str(i))

        X.columns = self.column_names
        return self

    def transform(self, X):
        return X.apply(self.len_special_characters)
